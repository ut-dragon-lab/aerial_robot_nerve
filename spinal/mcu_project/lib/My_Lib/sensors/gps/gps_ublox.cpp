...
void GPS::processMessage()
{
  switch (msg_id_)
  {
    case MSG_PVT:
    {
      ...
      state_.ground_course_cd = (raw_packet_.pvt.heading_2d / 1000);    // deg*100
      ...
      // Enqueue only (overwrite oldest if full). ROS layer drains and publishes.
      push_state_snapshot();
      break;
    }
    ...
    case MSG_VELNED:
    {
      ...
      state_.ground_course_cd = (raw_packet_.velned.heading_2d / 1000); // deg*100
