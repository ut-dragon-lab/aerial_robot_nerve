...
void KondoServo::init(UART_HandleTypeDef* huart,  osMutexId* mutex)
{
  ...
  HAL_UART_Receive_DMA(huart, dma_rx_buf_, KONDO_BUFFER_SIZE);

  std::memset(dma_rx_buf_, 0, sizeof(dma_rx_buf_));
  std::memset(pos_rx_buf_, 0, sizeof(pos_rx_buf_));
  ...
  if ( __HAL_UART_GET_FLAG(huart_, UART_FLAG_ORE) )
    {
      ...
      HAL_UART_Receive_DMA(huart_, dma_rx_buf_, KONDO_BUFFER_SIZE); // restart
    }
